...
public class ChangeHistoryService {
    ...
    public <T> @Nullable ConfigurationChange<T> getLatestByType(ConfigurationType configType) {
        var latest = configurationChangeRepository.findLatestByConfigType(configType.name());
        return latest.<ConfigurationChange<T>>map(ConfigurationChangeMapper::toDomain).orElse(null);
    }
}
